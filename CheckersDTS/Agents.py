...
class Node:
    ...
    def getUCBValue(self, depressed, agentTurn):
        condition = (not depressed) & (not (agentTurn ^ self.parent.turn))
    ...
    def rollout(self):
        ...
        for i in range(0, Global.ROLLOUT):
            ...
            board.make_move(moves[rnd.randint(0,len(moves)-1)])
        return board
...
class MCTS:
    ...
    def treePolicy(self):
        ...
        while not currentNode.board.is_over():
            if not currentNode.isFullyExpanded():
                currentNode = currentNode.expand()
                break
            currentNode = currentNode.select(self.depressed, self.root.turn)
        return currentNode;
    ...
    def getAction(self, board):
        self.root = Node(board.copy(), None)

        for i in range(0, Global.MAX_ITERS):
            newNode = self.treePolicy()
            tempBoard = newNode.rollout()
            newNode.backpropagate(Global.HEURISTIC(tempBoard, self.root.turn) - Global.HEURISTIC(board, self.root.turn))
